...
export class AuthService {
  ...
  async validateOAuthLogin(profile: { email: string, displayName: string }) {
    ...
    if (!user) {
      ...
      let usernameExists = await this.prisma.user.findUnique({ where: { username: baseUsername } })

      let counter = 1
      while (usernameExists) {
        baseUsername = baseUsername + counter
        usernameExists = await this.prisma.user.findUnique({ where: { username: baseUsername } })
        counter++
      }
